...
for i in range(pasos-1):
    Ustar = U[:,i] + dt/2 * ( D2@ U[:,i] - F2)
    U[:,i+1] = U[:,i] + dt * (D2@Ustar - F2)
...
if guarda_figuras:
    for i in indices:
        ...
        fig.savefig("figuras/16-RK22D-t-%1.3f.png" %t[i])
...
if make_video:
    ...
    animation.write_videofile('figuras/RK22D.mp4', fps=video_fps, verbose=False, logger=None)
